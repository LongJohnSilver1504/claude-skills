...
description: Write behavioral tests for feature modules using Vitest, React Testing Library, and MSW v2. Use when writing tests, adding test coverage, setting up test infrastructure (MSW handlers, shared factories, jsdom polyfills), or when the user says "write tests", "add coverage", "test this component/hook/feature". NOT for judging existing test quality inside the pipeline (the test-reviewer agent owns that gate) or finding the root cause of a failing test (systematic-debugging).
---
...
## Factories, Mapper Tests, and Contract Smoke

When a domain type appears in 3+ test files (or has 5+ required fields), use a **shared test factory** — two levels, mirroring the API boundary: **DTO factories** (wire shapes, `satisfies z.input<typeof xDtoSchema>`, feed MSW handlers) and **domain factories** (hand-authored domain types, feed component/hook/domain tests). Mappers always get direct unit tests — the cheapest tests in the codebase.

Read [references/factories.md](references/factories.md) for the full patterns (factory templates, MSW wiring, mapper tests, contract smoke) before writing factories or MSW handlers.
...
Read the feature's existing artifacts (`.claude/pipeline/{feature}/` and the feature folder: DESIGN.md, PRD.md, UX-spec.md, the implementation plan, PROGRESS.md — whichever exist), then the feature code and its existing test files, and continue from where they leave off — never restart from scratch.
